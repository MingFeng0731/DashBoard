...
if(isset($_POST['send']))
{
	$sql3 = "SELECT * FROM Car3 ORDER BY id DESC LIMIT 15";
	$result3 = mysqli_query($link,$sql3);
    $i=0;
    $list_arr=array();
    while($list=mysqli_fetch_array($result3)){  //判斷是否還有資料沒有取完，如果取完，則停止while迴圈。
    $list_arr[$i]=$list;
    $i++;
    }
	$err01 = $list_arr[0][1];
...
<iframe id="power" class="power"
             ...
             src="http://134.208.2.149:80/power_data3.html"
             frameborder="0">
</iframe>
//<script>
...
<iframe id="water" class="water"
             noresize="noresize"
             style="position: absolute; background-color: #272727; width: 35%; height:40%;"
             src="http://134.208.2.149:80/water_data3.html"
             frameborder="0">
</iframe>
<script>
    var map;
